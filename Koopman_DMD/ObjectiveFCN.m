 a sta  function J = ObjectiveFCN_EDMDc(uopt,x,N,Nu,xref,u0,p,Q,R,Ru)
% function J = ObjectiveFCN_EDMDc(uopt,x,N,Nu,xref,u0,p,Q,R,Ru,Xapxn)
...
%% Integrate system
  [xk,~] = lsim(p.sys,[u' 0],[0:N].*p.sys.Ts,Xapxn');
% [xk,~] = lsim(p.sys,[u' 0],[0:N].*p.sys.Ts,poolData(x',4,p.polyorder,p.usesine)');
xk = xk';
xk = xk(1:p.nvar,:);

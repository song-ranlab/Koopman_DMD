...
function [xHistory, uHistory, tHistory, rHistory] = runMPC(syshandle, Duration,Ts,N,Nu,x0, ObjectiveFCN, ConstraintFCN, Q, R, Ru, options, xref, pest, Xapx, firstu)
...
for ct = 1:Nt-1
    ...
    COSTFUN = @(u) ObjectiveFCN(u,xhat,N,Nu,xref,uHistory(:,ct),pest,Q,R,Ru,Xapxn);
%     COSTFUN = @(u) ObjectiveFCN_EDMDc(u,xhat,N,Nu,xref,uHistory(:,ct),pest,Q,R,Ru);
...
%     uopt = fmincon(COSTFUN,uopt,[],[],[],[],LB,UB,CONSFUN,option);
    ...
    % Integrate system
    xhat = myrk4u(syshandle,xhat,uopt(1),Ts/10,10,[],[]); %Using UW rk4u instead b/c ode45 is being dumb 
 %   [~,xhat] = ode45(syshandle,xhat,uopt(1)
    xHistory(:,ct+1) = xhat;

function [xhist,uhist,thist,rhist] = HBMPC_polyiu2(syshandle, Duration,Ts,N,Nu,x0, Q, R, Ru, xref, pest,uval)
...
%% Integrate system
[xk,~] = lsim(p.sys, [u' 0], [0:N].*p.sys.Ts, poolData([x;u0]',p.nvar+1,p.order,p.sine)');
...
%     %Hamiltoniian error
    J = J + (xk1-poolData([xref;uk]',p.nvar+1,p.order,p.sine)')'*Q*(xk1-poolData([xref;uk]',p.nvar+1,p.order,p.sine)'); %lift error
...
function x = myrk4u(f,x,u,h,n,t,p)
